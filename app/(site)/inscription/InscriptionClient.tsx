...
export default function InscriptionClient(){
  ...
  useEffect(()=>{
    ...
    const init=async()=>{
      ...
      const mesh=new THREE.Mesh(geo,new THREE.MeshStandardMaterial({color:0xc9a84c,metalness:0.93,roughness:0.07}));
      scene.add(mesh);
      const wire=new THREE.Mesh(geo,new THREE.MeshBasicMaterial({color:0xc9a84c,wireframe:true,transparent:true,opacity:0.1}));
      wire.scale.setScalar(1.02);scene.add(wire);
      const r=new THREE.Mesh(new THREE.TorusGeometry(1.8,0.01,8,100),new THREE.MeshBasicMaterial({color:0xc9a84c,transparent:true,opacity:0.18}));
      r.rotation.x=Math.PI/3;scene.add(r);
      scene.add(new THREE.AmbientLight(0xffffff,0.25));
      const l1=new THREE.PointLight(0xc9a84c,3,12);l1.position.set(4,3,3);scene.add(l1);
      const l2=new THREE.PointLight(0x4455ff,1,10);l2.position.set(-4,-2,2);scene.add(l2);
      const pGeo=new THREE.BufferGeometry();
      const pos=new Float32Array(120*3);
      for(let i=0;i<360;i++)pos[i]=(Math.random()-0.5)*10;
      pGeo.setAttribute("position",new THREE.BufferAttribute(pos,3));
      scene.add(new THREE.Points(pGeo,new THREE.PointsMaterial({color:0xc9a84c,size:0.02,transparent:true,opacity:0.55})));
      const clock=new THREE.Clock();
      const animate=()=>{
        aid=requestAnimationFrame(animate);
        const e=clock.getElapsedTime();
        mesh.rotation.y=e*0.2;mesh.rotation.x=Math.sin(e*0.15)*0.2;
        wire.rotation.y=e*0.2;wire.rotation.x=Math.sin(e*0.15)*0.2;
        r.rotation.z=e*0.09;
  ...
  const iCls="w-full px-4 py-3 rounded-xl text-sm transition-all duration-200";
  const iStyle={background:"rgba(255,255,255,0.04)",border:"1px solid rgba(201,168,76,0.2)",color:"#F5F0E8",outline:"none"};
  const onFocus=(e:React.FocusEvent<HTMLInputElement|HTMLTextAreaElement>)=>{e.target.style.border="1px solid rgba(201,168,76,0.6)";e.target.style.boxShadow="0 0 0 3px rgba(201,168,76,0.08)";};
  const onBlur=(e:React.FocusEvent<HTMLInputElement|HTMLTextAreaElement>)=>{e.target.style.border="1px solid rgba(201,168,76,0.2)";e.target.style.boxShadow="none";};
  ...
  return(
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative overflow-hidden" style={{background:"linear-gradient(135deg,#080808 0%,#0a0a0a 55%,#0d0b08 100%)"}}>
      <canvas ref={canvasRef} className="absolute inset-0 w-full h-full" style={{opacity:0.45,pointerEvents:"none"}}/>
      <div className="absolute inset-0 pointer-events-none" style={{background:"radial-gradient(ellipse 65% 55% at 50% 50%,rgba(201,168,76,0.05) 0%,transparent 70%)"}}/>
      <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.5}} className="relative z-10 w-full max-w-md">
        <div className="text-center mb-8" style={{perspective:"600px"}}>
          ...
          <motion.p initial={{opacity:0}} animate={{opacity:1}} transition={{delay:0.8}} className="text-sm font-medium" style={{color:"rgba(201,168,76,0.5)",letterSpacing:"0.1em"}}>{t("inscription_sous_titre")}</motion.p>
        </div>
        <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:0.3}} className="rounded-2xl p-8"
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {([
              {key:"nom",label:t("votre_nom"),type:"text",req:true},
              {key:"email",label:t("votre_email"),type:"email",req:true},
              {key:"telephone",label:t("votre_telephone"),type:"tel",req:false},
            ] as {key:keyof F;label:string;type:string;req:boolean}[]).map(({key,label,type,req})=>(
              <div key={key as string}>
                <label className="text-xs font-semibold uppercase tracking-widest mb-2 block" style={{color:"rgba(201,168,76,0.6)"}}>{label}{req?" *":""}</label>
                <input type={type} required={req} value={form[key] as string} onChange={(e)=>setForm({...form,[key]:e.target.value})} className={iCls} style={iStyle} onFocus={onFocus} onBlur={onBlur}/>
              </div>
            ))}
            {([
              {key:"motDePasse",label:t("mot_de_passe"),show:showPwd,toggle:()=>setShowPwd(!showPwd)},
              {key:"confirm",label:t("confirmer_mdp"),show:showCfm,toggle:()=>setShowCfm(!showCfm)},
            ] as {key:keyof F;label:string;show:boolean;toggle:()=>void}[]).map(({key,label,show,toggle})=>(
              <div key={key as string}>
                <label className="text-xs font-semibold uppercase tracking-widest mb-2 block" style={{color:"rgba(201,168,76,0.6)"}}>{label} *</label>
                <div className="relative">
                  <input type={show?"text":"password"} required value={form[key] as string} onChange={(e)=>setForm({...form,[key]:e.target.value})} className={`${iCls} pr-12`} style={iStyle} onFocus={onFocus} onBlur={onBlur}/>
                  <button type="button" onClick={toggle} className="absolute right-3 top-1/2 -translate-y-1/2 transition-colors" style={{color:"rgba(201,168,76,0.4)"}} onMouseEnter={(e)=>(e.currentTarget.style.color="#C9A84C")} onMouseLeave={(e)=>(e.currentTarget.style.color="rgba(201,168,76,0.4)")}><EyeIcon open={show}/></button>
                </div>
              </div>
            ))}
            ...
            <button type="submit" disabled={loading} className="w-full py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 disabled:opacity-50 mt-2"
              ...
              {loading?<span className="flex items-center justify-center gap-2"><svg className="w-4 h-4 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>{t("chargement")}</span>:t("creer_compte")}
            </button>
          </form>
          <p className="text-center text-sm mt-6" style={{color:"rgba(245,240,232,0.35)"}}>
            {t("deja_compte")}{" "}
            <Link href={`/connexion?redirect=${encodeURIComponent(redirect)}`} className="font-medium" style={{color:"#C9A84C"}}>{t("se_connecter")}</Link>
          </p>
        </motion.div>
        <p className="text-center mt-5">
          <Link href="/" className="text-sm transition-colors" style={{color:"rgba(201,168,76,0.35)"}} onMouseEnter={(e)=>(e.currentTarget.style.color="#C9A84C")} onMouseLeave={(e)=>(e.currentTarget.style.color="rgba(201,168,76,0.35)")}>← Retour à l'accueil</Link>
        </p>
      </motion.div>
    </div>
  );
